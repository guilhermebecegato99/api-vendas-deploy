...
class CreateOrderService {
    public async execute({ customer_id, products }: IRequest): Promise<Order> {
        ...
        if (!existsProducts.length) {
            throw new AppError('Could not find any products whith this ids!');
        }

        const existsProductsIds = existsProducts.map(product => product.id);

        const checkInexitentProducts = products.filter(
            product => !existsProductsIds.includes(product.id),
        );

        if (checkInexitentProducts.length) {
            throw new AppError(`Could not find product ${checkInexitentProducts[0].id}!`);
        }
